...
when parseEnum[MoneyUnit](moneyDefaultUnit) == moneyUnitCoins:
  ...
  suite "Money / comparisons + predicates":
    ...
    test "mismatched currencies raise MoneyError":
      expect MoneyError:
        discard m(100, USD) + m(100, EUR)
      expect MoneyError:
        discard m(100, USD) == m(100, EUR)
      expect MoneyError:
        discard m(100, USD) < m(100, EUR)
  ...
  suite "Money / conversion":
    test "init rates + convert":
      ...
      prevRates.add(EUR, eurRates)
      prevRates.add(GBP, gbpRates)
      prevRates.add(USD, usdRates)

      let eur = m(2000, EUR)
      let usd = eur.convert(USD, prevRates)
      let gbp = eur.convert(GBP, prevRates)
      check $usd == "USD 21.60"
      check $gbp == "GBP 17.10"

    test "convert edge cases":
      var rates = MoneyConversionRates()
      rates.add(EUR, parseJson("""{"EUR":1,"USD":1.0,"GBP":1.5}"""))
      let emptyRates = MoneyConversionRates()

      check m(2000).convert(USD, rates) == 2000.USD    # rate 1.0 = identity
      check $m(2000).convert(USD, rates) == "USD 20.00"
      check $m(2000).convert(GBP, rates) == "GBP 30.00" # rate 1.5 (single decimal)

      expect MoneyError:
        discard m(2000, GBP).convert(EUR, emptyRates)  # base GBP unregistered
      expect MoneyError:
        discard m(2000, EUR).convert(JPY, rates)       # no rate to JPY

    test "convert rounds half-up":
      var rates = MoneyConversionRates()
      rates.add(EUR, parseJson("""{"EUR":1,"GBP":1.5}"""))
      # 2001 units * 1.5 = 3001.5 -> rounds half-up to 3002 (GBP 30.02)
      check $m(2001).convert(GBP, rates) == "GBP 30.02"
    ...
    test "newMoney(float) and toMoney(float)":
      assert $newMoney(5.5) == "EUR 5.50"
      assert $newMoney(5.10) == "EUR 5.10"
      assert $newMoney(0.05) == "EUR 0.05"
      assert $newMoney(1234.56) == "EUR 1234.56"
      assert $newMoney(7.0) == "EUR 7.00"
      assert $toMoney(5.5) == "EUR 5.50"
      assert $toMoney(5.0) == "EUR 5.00"
      assert $toMoney(1234.56) == "EUR 1234.56"
      expect MoneyError:
        discard toMoney(5.123)  # more decimals than currency exponent

    test "avg":
      assert $avg(m(100), m(200), m(300)) == "EUR 2.00"
      assert $avg(m(100), m(101)) == "EUR 1.00"
    ...
    test "exchange":
      ...
      var prevRates = MoneyConversionRates()
      prevRates.add(EUR, eurRates["rates"])
      prevRates.add(GBP, gbpRates["rates"])
      prevRates.add(USD, usdRates["rates"])

      block:
        var x = newMoney(2000).convert(USD, prevRates) # EUR -> USD
        assert x == 2160.USD
      block:
        var xeur = 2000.EUR
        let xgbp = xeur.convert(GBP, prevRates)
        assert xgbp == 1710.GBP
        let xusd = xgbp.convert(USD, prevRates)
        assert xusd == 2155.USD  # 1710 * 1.26 = 2154.6, rounds half-up
